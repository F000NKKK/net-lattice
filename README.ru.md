...
Для отсутствующего в Windows native-мониторинга изменений соседей есть
опциональная, не-native замена: capability `Addition::NEIGHBOR_MONITORING_POLLING`,
которая используется через `Lattice::watch_with_additions`, а не
`watch`/`watch_filtered`. Это не меняет строку «Мониторинг изменений
соседей» выше — `Capability::NEIGHBOR_MONITORING` по-прежнему недоступна на
Windows, а `Addition` никогда не отображается обычной галочкой `Capability`
ни в этой, ни в какой-либо другой таблице по backend'ам (см. раздел
ARCHITECTURE.md «Platform Support Matrix and Gaps» о конвенции
предупреждающего маркера и более слабых гарантиях доставки addition —
латентность ограничена интервалом опроса, нет упорядоченности относительно
native-событий, возможна коалесценция противоположных изменений в пределах
одного интервала опроса). Бит `additions`, не заявленный
`AdditionProvider::additions()` подключённого backend'а, молча не
активируется, а не приводит к ошибке — так же, как и `Capability`,
